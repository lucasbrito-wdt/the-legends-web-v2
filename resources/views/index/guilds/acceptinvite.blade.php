...
<div class="container bg-border" style="margin-top:120px">
    ...
    <div class="bg-content">
        <div class="container p-0">
            <div class="bg-title">Aceitando Convite</div>
            @if($errorsView)
            ...
            <div class="main-content mb-2">
                ...
                <form id="formGuild" action="{{ route('guilds.acceptinvite', ['guildId' => $guild->getId()]) }}" method="POST" class="row d-flex flex-column align-items-center">
                    @csrf
                    <div class="col-5">
                        <div class="form-floating mb-1">
                            <select class="form-select @error('name') is-invalid @enderror" id="name" name="name" aria-label="Selecione um líder">
                              <option selected>Selecione o player</option>
                                @foreach($list_of_invited_players as $invited_player_from_list)
                                    <option value="{{ urlencode($invited_player_from_list) }}">{{ htmlspecialchars($invited_player_from_list) }}</option>
                                @endforeach
                            </select>
                            <label for="name">Selecione o player para se juntar-se a guild:</label>
                            @error('name')
            ...
            <div class="d-flex justify-content-center">
                <button id="submitGuild" type="submit" class="sbutton-blue">Aceitar</button>
