    ...
    <div class="bg-content">
        <div class="container p-0">
            ...
            <div class="main-content mb-2">
                <form id="formGuild" action="{{ route('guilds.deleteguild', ['guildId' => $guild->getId()]) }}" method="POST" class="row d-flex flex-column align-items-center">
                    @csrf
                    <div class="col-5">
                        <p class="text-center m-0">Tem certeza que deseja deletar a guilda com ID <b>{{ $guild->getId() }}</b>?</p>
                    </div>
                </form>
            </div>
            <div class="d-flex justify-content-center">
                <button id="submitGuild" type="submit" class="sbutton-blue">Sim, deletar</button>
                <a href="{{ route('guilds.manager', ['guildId' => $guild->getId() ]) }}" class="sbutton-red">Voltar</a>
            </div>
            @endif
        </div>
    </div>
